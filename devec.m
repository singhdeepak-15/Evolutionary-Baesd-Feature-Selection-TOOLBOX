function [bestmem,bestval,nfeval] = devec(fname,VTR,D,XVmin,XVmax,y,NP,itermax,clas,F,CR,strategy,refresh)
...
for i=1:NP
   pop(i,:) = ceil(XVmin + rand(1,D).*(XVmax - XVmin));
end
...
val(1)  = feval(fname,pop(ibest,:),y,clas);
bestval = val(1);                 % best objective function value so far
nfeval  = nfeval + 1;
for i=2:NP                        % check the remaining members
  val(i) = feval(fname,pop(i,:),y,clas);
  nfeval  = nfeval + 1;
...
while ((iter < itermax)) % (bestval > VTR))
  ...
  if (st == 1)                      % DE/best/1
    ...
    ui = abs(ceil(popold.*mpo + ui.*mui));     % crossover
...
%-----Select which vectors are allowed to enter the new population------------
  for i=1:NP
    tempval = feval(fname,ui(i,:),y,clas);   % check cost of competitor
    nfeval  = nfeval + 1;
